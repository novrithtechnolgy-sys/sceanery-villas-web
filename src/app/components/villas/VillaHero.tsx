...
import { FaBed, FaMountain, FaSwimmingPool, FaUsers } from "react-icons/fa";
  ...
  return (
    <section>
      <div className="relative overflow-hidden rounded-t-[28px] md:rounded-b-[28px] h-[420px] md:h-[520px] lg:h-[600px]">
        {heroUrl && <Image src={heroUrl} alt={villa.title} fill priority className="object-cover" />}
      ...
      <Container>
        <div className="rounded-[22px] max-w-5xl mx-auto p-4 md:p-6 z-10">
          <div className="grid md:grid-cols-2 gap-4 md:gap-10">
            ...
            <Stat
              icon={<FaBed />}
              label={`${villa.stats?.bedrooms ?? "-"} Bedrooms`}
            />

            <Stat
              icon={<FaUsers />}
              label={`Sleeps ${villa.stats?.sleeps ?? "-"}`}
            />

            <Stat
              icon={<FaSwimmingPool />}
              label={villa.stats?.pool ?? "Pool"}
            />

            <Stat
              icon={<FaMountain />}
              label={villa.stats?.views ?? "Views"}
            />
