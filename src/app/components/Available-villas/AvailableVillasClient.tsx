...
export default function AvailableVillasClient() {
  ...
  const fetchVillas = async () => {
    try {
      ...
      setVillas([]);
      setError("Something went wrong while loading available villas. Please try again.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <Container>
        <div className="mt-12 space-y-10">
          ...
          {!loading && !error && noResults && (
            <div className="bg-white text-center flex flex-col items-center justify-center py-20 rounded-[22px] border border-gray-200">
              <h3 className="text-[22px] font-semibold text-gray-900">
                No available villas found
              </h3>
              <p className="mt-3 text-gray-600 text-[16px] max-w-xl">
                There are no villas available for the selected dates and guest count.
